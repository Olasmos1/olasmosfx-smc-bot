...
SIGNAL_SCAN_INTERVAL = 60 * 15   # scan for new signals every 15 mins
PRICE_CHECK_INTERVAL = 60        # check TP/SL every 1 minute
...
# --- Fast Price Check Loop (every 1 min) ---
async def price_monitor_loop(bot: Bot):
    """Checks TP/SL every minute for fast alerts"""
    await asyncio.sleep(20)  # wait for startup
    while True:
        try:
            if has_open_trade():
                df = fetch_data(symbol="XAU_USD", interval="M1", count=5)
                if df is not None:
                    ...
                    if closed:
                        ...
                        logger.info(f"Fast exit alert: {closed['result']} at {current_price}")
                        # Immediately scan for next signal after TP/SL
                        await signal_scan(bot)
        except Exception as e:
...
# --- Signal Scan ---
async def signal_scan(bot: Bot):
    """Scan for new signal — only runs when no open trade"""
    if has_open_trade():
        logger.info("Trade open — skipping signal scan")
        return
...
# --- Signal Scan Loop (every 15 mins) ---
...
async def run_bot():
    ...
    try:
        ...
        logger.info("Webhook cleared — push-only mode")
    except Exception as e:
