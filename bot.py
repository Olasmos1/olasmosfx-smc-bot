...
def format_signal(signal):
    ...
    # Grade label
    grade_labels = {1: "L1 • Weak", 2: "L2 • Moderate", 3: "L3 • Good", 4: "L4 • Strong", 5: "L5 • Very Strong"}
...
def signal_key(signal):
    """Unique key for a signal to prevent duplicates"""
    return f"{signal['direction']}_{signal['entry']}_{signal['grade']}"


async def scan_and_send(bot: Bot):
    """Main scan loop - fetch data, analyze, send signals"""
    logger.info("Running scan...")

    df = fetch_data(SYMBOL, interval="15m", period="5d")
    if df is None:
    ...
    for signal in signals:
        ...
        try:
            ...
            # Keep sent_signals from growing forever
...
async def post_init(app: Application):
    """Start background scan loop after bot initializes"""
